    ...
    public class ViewModelAlumno : BaseViewModel
    {
        ...
        public async Task ViewCalcPromedio()
        {
            if(alumnoActual.tipoAlumno == 1)
            {
                ...
                await Application.Current.MainPage.Navigation.PushAsync(new ViewCalcDiasSemana(alumnoActual));
            }else if(alumnoActual.tipoAlumno == 2)
            {
              
                await Application.Current.MainPage.Navigation.PushAsync(new ViewCalcNotaFinde(alumnoActual));
            }
        }
